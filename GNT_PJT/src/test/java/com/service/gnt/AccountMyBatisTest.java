...
public class AccountMyBatisTest {
	...
	public void depositAcc() throws Exception {
		...
		SqlSession session = factory.openSession();
		Users user = new Users("aa@naver.com", "AAA", "12355");
		int userId = session.selectOne(um+"getUserId",user);
		String accId = session.selectOne(um+"getAccIdByUserId",userId);
		System.out.println("사용자의 계좌로 특정 금액을 입금합니다.");
		session.update(am + "manageAcc", new Account(accId, 5000, 0));
		int message = session.selectOne(am + "getAccBalance", accId);
	...
	public void createMile() throws Exception{
		...
		session.insert(am + "createMile", accId);
